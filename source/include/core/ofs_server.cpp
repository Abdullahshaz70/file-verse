// #include <iostream>
// #include <thread>
// #include <sstream>
// #include <vector>
// #include <string>

// #include <sys/socket.h>
// #include <arpa/inet.h>
// #include <netinet/in.h>
// #include <unistd.h>

// #include "user_manager.hpp"
// #include "session_manger.hpp"
// #include "OFSCore.hpp"
// #include <limits>

// // ======== GLOBAL SHARED OBJECTS =========
// UserManager gUserMgr;
// OFSCore gOFS(&gUserMgr, 2048);


// using namespace std;
// static const int SERVER_PORT = 9090;

// // -----------------------------------
// // Helper split
// vector<string> split(const string& s, char delim) {
//     vector<string> parts;
//     string temp;
//     stringstream ss(s);
//     while (getline(ss, temp, delim)) parts.push_back(temp);
//     return parts;
// }

// // -----------------------------------
// void handleClient(int clientSock) {
//    SessionManager session(&gUserMgr);
//     gOFS.attachSession(&session);


//     char buffer[8192];

//     while (true) {
//         ssize_t n = recv(clientSock, buffer, sizeof(buffer), 0);
//         if (n <= 0) break;

//         buffer[n] = '\0';
//         string req(buffer);
//         req.erase(remove(req.begin(), req.end(), '\n'), req.end());

//         auto parts = split(req, '|');
//         string cmd = parts[0];

//         string reply = "";

//         if (cmd == "QUIT") {
//             reply = "OK|BYE\n";
//             send(clientSock, reply.c_str(), reply.size(), 0);
//             break;
//         }

//         else if (cmd == "LOGIN") {
//             bool ok = session.login(parts[1], parts[2]);
//             reply = ok ? "OK|LOGIN\n" : "ERR|LOGIN_FAILED\n";
//         }

//         else if (cmd == "LOGOUT") {
//             session.logout();
//             reply = "OK|LOGOUT\n";
//         }

//         else if (cmd == "FORMAT") {
//             gOFS.format();
//             reply = "OK|FORMATTED\n";
//         }
//         else if (cmd == "LOAD") {
//             bool ok = gOFS.loadSystem();
//             reply = ok ? "OK|LOADED\n" : "ERR|LOAD_FAILED\n";
//         }


//         else if (cmd == "CREATE_USER") {
//             gOFS.createUser(parts[1], parts[2], parts[3] == "1");
//             reply = "OK|USER_CREATED\n";
//         }

//         else if (cmd == "CREATE_FILE") {
//             gOFS.createFile(parts[1], parts[2]);
//             reply = "OK|FILE_CREATED\n";
//         }

//         else if (cmd == "DELETE_FILE") {
//             bool ok = gOFS.deleteFile(parts[1]);
//             reply = ok ? "OK|FILE_DELETED\n" : "ERR|DELETE_FAILED\n";
//         }

//         else if (cmd == "READ_BLOCK") {
//             stringstream out;
//             streambuf* old = cout.rdbuf(out.rdbuf());

//             gOFS.readFileContent(stoi(parts[1]), 4096);

//             cout.rdbuf(old);
//             reply = out.str();
//         }


//         else if (cmd == "CREATE_DIR") {
//             gOFS.createDirectory(parts[1]);
//             reply = "OK|DIR_CREATED\n";
//         }

//         else if (cmd == "DELETE_DIR") {
//             bool ok = gOFS.deleteDirectory(parts[1]);
//             reply = ok ? "OK|DIR_DELETED\n" : "ERR|DELETE_FAILED\n";
//         }

//         else if (cmd == "LIST_MY_FILES") {
//             // capture cout into string
//             stringstream out;
//             streambuf* old = cout.rdbuf(out.rdbuf());

//             gOFS.listMyFiles();

//             cout.rdbuf(old);
//             reply = out.str();
//         }

//         else if (cmd == "LIST_ALL_FILES") {
//             stringstream out;
//             streambuf* old = cout.rdbuf(out.rdbuf());
//             gOFS.listAllFiles();
//             cout.rdbuf(old);
//             reply = out.str();
//         }

//         else if (cmd == "SHOW_TREE") {
//             stringstream out;
//             streambuf* old = cout.rdbuf(out.rdbuf());
//             gOFS.showMyDirectoryTree();
//             cout.rdbuf(old);
//             reply = out.str();
//         }

//         else if (cmd == "SHOW_CHANGE_LOG") {
//             stringstream out;
//             streambuf* old = cout.rdbuf(out.rdbuf());
//             gOFS.showChangeLog();
//             cout.rdbuf(old);
//             reply = out.str();
//         }

//         else reply = "ERR|UNKNOWN_COMMAND\n";

//         send(clientSock, reply.c_str(), reply.size(), 0);
//     }

//     close(clientSock);
// }

// // -----------------------------------
// int start_server() {
//     int sock = socket(AF_INET, SOCK_STREAM, 0);
//     int opt = 1;
//     setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

//     sockaddr_in addr{};
//     addr.sin_family = AF_INET;
//     addr.sin_port = htons(9090);
//     addr.sin_addr.s_addr = INADDR_ANY;

//     bind(sock, (sockaddr*)&addr, sizeof(addr));
//     listen(sock, 10);

//     cout << "🚀 OFS SERVER running on port 9090\n";

//     SessionManager boot(&gUserMgr);
//     gOFS.attachSession(&boot);
//     gOFS.loadSystem();


//     while (true) {
//         sockaddr_in client{};
//         socklen_t len = sizeof(client);
//         int c = accept(sock, (sockaddr*)&client, &len);

//         thread(handleClient, c).detach();
//     }
// }


#include <iostream>
#include <thread>
#include <sstream>
#include <vector>
#include <string>

#include <sys/socket.h>
#include <arpa/inet.h>
#include <netinet/in.h>
#include <unistd.h>

#include "user_manager.hpp"
#include "session_manger.hpp"
#include "OFSCore.hpp"

using namespace std;

static const int SERVER_PORT = 9090;

// ================= GLOBAL SHARED OBJECTS =================
UserManager gUserMgr;           // Shared for all clients
OFSCore     gOFS(&gUserMgr);    // One OFS core (you asked for this)

// ---- Safe way to attach session for ONE command only ----
#define WITH_SESSION(sess) gOFS.attachSession(sess)

// ====================== HELPERS ==========================
vector<string> split(const string& s, char delim) {
    vector<string> p;
    string temp;
    stringstream ss(s);
    while (getline(ss, temp, delim)) p.push_back(temp);
    return p;
}

// ===================== CLIENT THREAD ======================
void handleClient(int clientSock) {
    SessionManager session(&gUserMgr);   // private session per client
    char buffer[8192];

    while (true) {
        ssize_t n = recv(clientSock, buffer, sizeof(buffer)-1, 0);
        if (n <= 0) break;

        buffer[n] = '\0';
        string req(buffer);
        req.erase(remove(req.begin(), req.end(), '\n'), req.end());

        auto parts = split(req, '|');
        string cmd = parts[0];
        string reply = "";

        // -------- QUIT --------
        if (cmd == "QUIT") {
            reply = "OK|BYE\n";
            send(clientSock, reply.c_str(), reply.size(), 0);
            break;
        }

        // -------- LOGIN --------
        else if (cmd == "LOGIN") {
            bool ok = session.login(parts[1], parts[2]);
            reply = ok ? "OK|LOGIN\n" : "ERR|LOGIN_FAILED\n";
        }

        // -------- LOGOUT --------
        else if (cmd == "LOGOUT") {
            session.logout();
            reply = "OK|LOGOUT\n";
        }

        // -------- FORMAT (ADMIN) --------
        else if (cmd == "FORMAT") {
            WITH_SESSION(&session);
            gOFS.format();
            reply = "OK|FORMATTED\n";
        }

        // -------- LOAD --------
        else if (cmd == "LOAD") {
            WITH_SESSION(&session);
            reply = gOFS.loadSystem() ? "OK|LOADED\n" : "ERR|LOAD_FAILED\n";
        }

        // -------- CREATE USER (Admin) --------
        else if (cmd == "CREATE_USER") {
            WITH_SESSION(&session);
            gOFS.createUser(parts[1], parts[2], parts[3] == "1");
            reply = "OK|USER_CREATED\n";
        }

        // -------- CREATE FILE --------
        else if (cmd == "CREATE_FILE") {
            WITH_SESSION(&session);
            gOFS.createFile(parts[1], parts[2]);
            reply = "OK|FILE_CREATED\n";
        }

        // -------- DELETE FILE --------
        else if (cmd == "DELETE_FILE") {
            WITH_SESSION(&session);
            bool ok = gOFS.deleteFile(parts[1]);
            reply = ok ? "OK|FILE_DELETED\n" : "ERR|DELETE_FAILED\n";
        }

        // -------- READ BLOCK --------
        else if (cmd == "READ_BLOCK") {
            WITH_SESSION(&session);
            stringstream out;
            streambuf *old = cout.rdbuf(out.rdbuf());

            gOFS.readFileContent(stoi(parts[1]));

            cout.rdbuf(old);
            reply = out.str();
        }

        // -------- CREATE DIR --------
        else if (cmd == "CREATE_DIR") {
            WITH_SESSION(&session);
            gOFS.createDirectory(parts[1]);
            reply = "OK|DIR_CREATED\n";
        }

        // -------- DELETE DIR --------
        else if (cmd == "DELETE_DIR") {
            WITH_SESSION(&session);
            bool ok = gOFS.deleteDirectory(parts[1]);
            reply = ok ? "OK|DIR_DELETED\n" : "ERR|DELETE_FAILED\n";
        }

        // -------- LIST MY FILES --------
        else if (cmd == "LIST_MY_FILES") {
            WITH_SESSION(&session);
            stringstream out;
            streambuf *old = cout.rdbuf(out.rdbuf());

            gOFS.listMyFiles();

            cout.rdbuf(old);
            reply = out.str();
        }

        // -------- LIST ALL FILES --------
        else if (cmd == "LIST_ALL_FILES") {
            WITH_SESSION(&session);
            stringstream out;
            streambuf *old = cout.rdbuf(out.rdbuf());

            gOFS.listAllFiles();

            cout.rdbuf(old);
            reply = out.str();
        }

        // -------- SHOW TREE --------
        else if (cmd == "SHOW_TREE") {
            WITH_SESSION(&session);
            stringstream out;
            streambuf *old = cout.rdbuf(out.rdbuf());

            gOFS.showMyDirectoryTree();

            cout.rdbuf(old);
            reply = out.str();
        }

        // -------- SHOW CHANGE LOG --------
        else if (cmd == "SHOW_CHANGE_LOG") {
            WITH_SESSION(&session);
            stringstream out;
            streambuf *old = cout.rdbuf(out.rdbuf());

            gOFS.showChangeLog();

            cout.rdbuf(old);
            reply = out.str();
        }

        // -------- UNKNOWN --------
        else {
            reply = "ERR|UNKNOWN_COMMAND\n";
        }

        send(clientSock, reply.c_str(), reply.size(), 0);
    }

    close(clientSock);
}

// ====================== SERVER MAIN ========================
int start_server() {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    int opt = 1;
    setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

    sockaddr_in addr{};
    addr.sin_family = AF_INET;
    addr.sin_port   = htons(SERVER_PORT);
    addr.sin_addr.s_addr = INADDR_ANY;

    bind(sock, (sockaddr*)&addr, sizeof(addr));
    listen(sock, 50);

    cout << "🚀 OFS SERVER running on port " << SERVER_PORT << "\n";

    // Load filesystem at server boot
    {
        SessionManager boot(&gUserMgr);
        WITH_SESSION(&boot);
        gOFS.loadSystem();
    }

    while (true) {
        sockaddr_in client{};
        socklen_t len = sizeof(client);
        int clientSock = accept(sock, (sockaddr*)&client, &len);
        thread(handleClient, clientSock).detach();
    }
}
